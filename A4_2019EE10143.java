...
public class A4_2019EE10143 {
    ...
    public static HashMap<String, HashMap<String, Integer>> nodeDelimiter(String file, HashMap<String, HashMap<String, Integer>> graph) throws Exception {
        ...
        while( (input = parse.readLine()) != null) {
            ...
            for (int curr = 0; curr<input.length();curr++) {
                if (input.charAt(curr)=='\"') toggle = !toggle; // Basically acts like a stack. Back to og when hits even quotes (Similar to parenthesis in stacks)
                else if (input.charAt(curr) == ',' && !toggle) {
                    result.add(input.substring(start,curr));
                    start = curr+1;
                }
            }
            result.add(input.substring(start));
            // String[] head = result;
            result.set(0, result.get(0).replaceAll("\"(.+)\"", "$1"));
            result.set(1, result.get(1).replaceAll("\"(.+)\"", "$1"));
    ...
    public static HashMap<String, HashMap<String, Integer>> edgeDelimiter(String file, HashMap<String, HashMap<String, Integer>> graph) throws Exception {
	    ...
	    result.set(0, result.get(0).replaceAll("\"(.+)\"", "$1"));
            ...
            // head[0] = head[0].replaceAll("\"(.+)\"", "$1");
            // head[1] = head[1].replaceAll("\"(.+)\"", "$1");
            ...
            String strWt = result.get(2);
            int weight = Integer.parseInt(strWt);
	        // if (graph.get(source).containsKey(target)) graph.get(source).put(target, graph.get(source).get(target) + weight); 
            graph.get(source).put(target, weight);
            // if (graph.get(target).containsKey(source)) graph.get(target).put(source, graph.get(target).get(source) + weight); 
    ...
    public static void Randomsort(int[] occurences, String[] chars, int left, int right){
        ...
        if (left<right){
            ...
            Randomsort(occurences, chars, left, p);
